...
export default function TabSubjects() {
  ...
  const del = async (id, name) => {
    const ok = await swalConfirm({ title: `ลบวิชา "${name}"?`, confirmText: "ลบ", danger: true }); if (!ok) return;
  ...
  return (
    <div className="row g-4">
      ...
      <div className="col-12 col-lg-7">
        <div className="card card-pink">
          <div className="card-header">
            <i className="bi bi-book me-2" />รายวิชา ({subjects.length} วิชา)
          </div>
          <div className="card-body p-0">
            {subjects.length === 0
              ? <div className="text-center py-5 text-muted"><i className="bi bi-book fs-1 d-block mb-2" />ยังไม่มีวิชา กรุณาเพิ่มวิชา</div>
              : (
